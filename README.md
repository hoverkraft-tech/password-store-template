...
Follow instructions at the official site : <https://www.passwordstore.org/>
...
The `PASSWORD_STORE_DIR` environment variable needs to point to the password-store repository. This is done for you
automatically in repositories that need the password-store through a `.env` file.

- To generate a new key pair for a user:

```sh
foo@bar:~/password-store$ gpg --generate-key
[...]
...
```sh
user@hostname:~$ gpg --export -a User Name > uname.asc
```
...
```sh
foo@bar:~/password-store/gpg-keys$ gpg --import *
```

Change the gpg trust level of the imported keys to 5 (maximum trust):

```sh
foo@bar:~/password-store$ gpg --list-keys
...
foo@bar:~/password-store$ gpg --edit-key AAE276AE5745E13C95336B263204648F2C48EA42...
...
- To add/update new entries to the password store:
  ...
  - Execute: `pass init -p . $(cat .gpg-id)`

```sh
# example
foo@bar:~/password-store$ pass insert dev/databases/main
...
```sh
# example
foo@bar:~/password-store$ pass insert -m some/secret/pass
```

Of if you havec a more complex object to store, you can use
